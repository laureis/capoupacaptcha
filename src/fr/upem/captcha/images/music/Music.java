...
public class Music implements Images {
    ...
    public ArrayList<CaptchaImage> getImages(int number) {
        ...
        CaptchaFilesManager.getAllFilesWithExtension(new File("src"+File.separator+"fr"+File.separator+"upem"+File.separator+"captcha"+File.separator+"images"+File.separator+"food"), "png", allImages);
        ...
        for (int i = 0; i < 6 - number; i++) {
            ...
            if (!allImagesTheme.contains(allImages.get(i))) someImages.add(allImages.get(i));
                ...
                while (allImagesTheme.contains(allImages.get(i))) {
                    randomIndex = ThreadLocalRandom.current().nextInt(6, allImages.size());
                }
